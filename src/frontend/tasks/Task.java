...
public abstract class Task implements Serializable {
	...
	@Deprecated
	// function that tries to evaluates the value (if necessary) and returns the
	// real value
	...
	@Deprecated
	protected Textfield createGuiTextField(String target, int localx, int localy, int w, Group g,
			CallbackListener callback) {
		return (cp5.addTextfield(get_gui_id() + "/" + target).setPosition(localx, localy)
				.setSize(w, (int) (font_size * 1.25)).setGroup(g).setAutoClear(false).setLabel(target)
				.align(ControlP5.CENTER, ControlP5.CENTER, ControlP5.CENTER, ControlP5.BOTTOM_OUTSIDE)
				.onClick(callbackEmptyWhenUsingUserInput(target)).onChange(callback).onReleaseOutside(callback));
	}

	@Deprecated
	protected ScrollableList createScrollableList(String name, List list, int localx, int localy, int w, Group g,
			CallbackListener callback) {
		return cp5.addScrollableList(get_gui_id() + "/" + name).setPosition(localx, localy).setLabel(name)
				.setSize(w, 100).setGroup(g).setDefaultValue(2).close().setValue(1).setBarHeight(20).setItemHeight(20)
				.align(ControlP5.CENTER, ControlP5.CENTER, ControlP5.CENTER, ControlP5.BOTTOM_OUTSIDE)
				.onChange(callback).setType(ControlP5.DROPDOWN).addItems(list);
	}
	...
	public Group load_gui_elements(State s) {
		...
		Group g = cp5.addGroup(g_name).setHeight((int) (font_size * 1.5f))
				...
				.setWidth(10 * font_size).setColorBackground(p.color(255, 50)) // color
																				// of
																				// the
																				// task
				.setBackgroundColor(p.color(255, 25)) // color of task when
														// openned
		;

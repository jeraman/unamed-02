...
public class ScriptingTask extends Task {
	...
	void set_context() {
		...
		// context.setAttribute("board", ((ZenStates)p).board,
	...
	void update_context() {
		...
		for (String val : ordered) {
			// drawItem(val, this.get(val), x, y+(myheight*(i+1))+i+1, mywidth,
			// myheight);
	...
	void write_in_new_file() {
		...
		try {
			...
			writer.write("/************************************************** \n"
					...
					+ "*************************************************** \n" + "*** created in: " + p.day() + "/"
					+ p.month() + "/" + p.year() + " - " + p.hour() + "h:" + p.minute() + "min *************** \n"
					+ "***************************************************/ \n\n"
	...
	public Group load_gui_elements(State s) {
		/*
		 * // TODO Auto-generated method stub CallbackListener cb_enter =
		 * generate_callback_enter(); CallbackListener cb_pressed =
		 * generate_callback_open_file();
		 * 
		 * int c1 = p.color(255, 50); int c2 = p.color(255, 25);
		 * 
		 * String g_name = this.get_gui_id();
		 * 
		 * String textlabel = "Scripting"; int font_size =
		 * (int)(((ZenStates)p).get_font_size()); int textwidth =
		 * (int)((ZenStates)p).textWidth(textlabel); int backgroundheight =
		 * (int)(font_size* 10.5);
		 * 
		 * Group g = cp5.addGroup(g_name) //.setPosition(x, y) //change that?
		 * .setHeight(font_size) .setWidth((10*((ZenStates)p).FONT_SIZE))
		 ...
		 * 
		 * int localx = 10, localy = (int)(font_size), localoffset =
		 * 3*font_size; int w = g.getWidth()-(localx*2);
		 * 
		 */
		...
		cp5.addTextfield(g_name + "/filename").setPosition(localx, localy).setSize(w, (int) (font_size * 1.25))
				.setGroup(g).setAutoClear(false).setLabel("filename").setText(this.name).onChange(cb_enter)

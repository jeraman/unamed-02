...
public class Connection implements Serializable {
	...
	ConnectionTextfieldUi condition;
	...
	void update_expression(Expression exp) {
		this.condition.setValue(exp.toString());
	}
	...
	String get_name() {
		// p.println("connection " + parent.get_id() + "_TO_" +
		// next_state.get_id() + " expr: " + this.expression.toString());
	...
	void init_gui_items() {
		...
		DropdownList d = cp5.addDropdownList(gui_name + "/priority").setWidth(ConnectionTextfieldUi.width).setItemHeight(20).setBarHeight(15)
				.setBackgroundColor(background).setColorBackground(background).setOpen(false).setValue(this.priority)
				.setLabel(this.priority + "").onChange(generate_callback_dropdown());
...
//		Textfield t = cp5.addTextfield(gui_name + "/condition").setText(text).setColorValue(white)
//				.setColorBackground(background).setColorForeground(background).setWidth(get_label_width() - w)
//				.setHeight(15).setFocus(false).setAutoClear(false).setLabel("")
//
//		;
//
...
//			return (int) (w + (p.textWidth(this.expression.toString()) * 1.3));
	...
	boolean is_mouse_over() {
		...
		// if it's a transition to self, and there are no dropdownlist or
		// textfields
		//if (d == null | t == null)
		if (d == null)
			return false;
...
//		return t.isMouseOver() || d.isMouseOver();
		return condition.isMouseOver() || d.isMouseOver();
	}
...
//					t.setText(get_expression().toString());
//				// if the name didn't change, no need to continue
//				if (t.getText().equalsIgnoreCase(get_expression().toString()))
//					return;
//
